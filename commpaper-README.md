...
_* the smart contract uses these (along with business logic) to decide outcomes; some of these inputs change the asset like _'ownership'_ or _'status'_ ; some don't e.g. _'price_' (the transaction inputs that took place during a commercial paper's lifecycle, are still recorded in the individual transaction's history on the blockchain though)_
